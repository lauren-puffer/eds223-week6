...
```{r}
# check that boundary is a polygon
...
```{r}
# change boundary from a polygon to a line

# update the name for the polygon version
park_polygon <- boundary

# create 'line version' of park boundary
park_border <- boundary |>
  st_cast("MULTILINESTRING") # converts a geometry to another type

# "." in st_cast() function is a placeholder for the object you are piping 
```
...
# transform projected crs
cycle_hire_osm_proj <- st_transform(cycle_hire_osm, "EPSG:27700")

# define raster template
template_raster <- rast(ext(cycle_hire_osm_proj), resolution = 1000, 
                        crs=crs(cycle_hire_osm_proj))
```
...
```{r}
ch_raster1 <- rasterize(cycle_hire_osm_proj, template_raster)
```
...
```{r}
# get raster of # of cycle hire points in each cell
ch_raster2 <- rasterize(cycle_hire_osm_proj, template_raster, fun = "length")
```
...
```{r}
# sum each of the points' capacity
ch_raster3 <- rasterize(cycle_hire_osm, template_raster, field = "capacity", 
                        fun = sum , na.rm = TRUE)
```
...
```{r}
#| message: false
map1 <- tm_shape(cycle_hire_osm_proj) +
...
# apply NDVI function o LANDSAT bands 3 & 4
...
# plot all maps
tmap_arrange(map1, map3, ndwi_map)
```

Normalized Difference Moisture Index: vegetation index used in RS to measure
and monitor the water content in vegetation. 
...
# arrange
tmap_arrange(map1, map3, ndmi_map)
```
